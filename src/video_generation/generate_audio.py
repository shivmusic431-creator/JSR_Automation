...
class AudioGenerationOrchestrator:
    ...
    def _assemble_script_from_sections(self, script_obj: Dict) -> str:
        ...
        Supports both long-form and shorts format.
        """
        sections = []

        # --- SHORTS FORMAT (hook_line + main_points + cta_line) ---
        if 'hook_line' in script_obj:
            log("📱 Detected SHORTS script format")
            if script_obj.get('hook_line'):
                sections.append(script_obj['hook_line'])
            for point in script_obj.get('main_points', []):
                if isinstance(point, dict) and point.get('narration'):
                    sections.append(point['narration'])
                elif isinstance(point, str):
                    sections.append(point)
            if script_obj.get('cta_line'):
                sections.append(script_obj['cta_line'])
            complete_script = ' [PAUSE-1] '.join(sections)
            log(f"  ✓ Assembled {len(sections)} shorts sections")
            return complete_script

        # --- LONG FORMAT ---
        log("🎬 Detected LONG-FORM script format")
